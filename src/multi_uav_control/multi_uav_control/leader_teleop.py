...
AXIS_RIGHT_STICK_LR = 3     # Right stick Left/Right → Strafe (linear-y)
AXIS_RIGHT_STICK_UD = 4     # Right stick Up/Down → Forward/Back (linear-x)

# Button mappings (Xbox controller default)
BUTTON_A = 0                # A button → ARM
BUTTON_X = 3                # X button → Enable auto-hover (throttle = 0.5)
BUTTON_Y = 2                # Y button → Emergency stop (disarm + zero throttle)
...
AXIS_DEADZONE = 0.01
THROTTLE_DEADZONE = 0.1    # Larger deadzone for throttle stick
...
class JoystickTeleop(Node):
    def __init__(self):
        ...
        self.joy_received = False  # Don't publish until joystick actually connects
    ...
    def publish_velocity(self):
        ...
        if self.disarm_requested:
            return
        # Don't publish zeros until a physical joystick is actually connected
